...
# Details of study
The anlaysis included data on up to `r nrow(oecd)` countries. The following lists the countries.
...
```{r life_satisfaction_correlations}

other_variables <- names(oecd)[!names(oecd) %in% c('COUNTRY', 'Life.Satisfaction')]
lifesat_correlations <- cor(oecd$Life.Satisfaction, oecd[,other_variables], use="pair")[1,]
lifesat_correlations <- data.frame(r=round(lifesat_correlations, 2))
...
lifesat_correlations[order(lifesat_correlations$abs_r, decreasing=TRUE), c('r', 'n')] 
```
